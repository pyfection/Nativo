...
async def list_languages(
    skip: int = 0, limit: int = 100, is_endangered: bool = None, db: Session = Depends(get_db)
):
    """
...
async def create_language(
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Language with this name already exists"
        )

    # Check if ISO code already exists (if provided)
    if language_data.iso_639_3:
        existing_iso = (
            db.query(Language).filter(Language.iso_639_3 == language_data.iso_639_3).first()
        )
        if existing_iso:
            raise HTTPException(
...
async def delete_language(
    language_id: UUID, current_user: User = Depends(require_admin), db: Session = Depends(get_db)
):
    """
...
class WritingStandardUpdate(BaseModel):
    """Promote a Document to be this language's canonical writing standard.

    Pass `null` to clear (effectively un-publish the standard).
    """

    document_id: UUID | None = None


@router.put("/{language_id}/writing-standard", response_model=LanguageSchema)
async def set_writing_standard(
    language_id: UUID,
    payload: WritingStandardUpdate,
    current_user: User = Depends(require_admin),
    db: Session = Depends(get_db),
):
    """Admin-only: set or clear the writing-standard Document for a language."""
    language = db.query(Language).filter(Language.id == language_id).first()
    if language is None:
    ...
    language.writing_standard_document_id = payload.document_id
    db.commit()
    db.refresh(language)
    return language
