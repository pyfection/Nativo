...
def is_transient_error(error_msg: str, error_type: str = 'general') -> bool:
    ...
    transient_keywords = {
        'general': [
            ...
            'temporary', '503', '502', '504', '429'  # Service unavailable, Bad Gateway, Gateway Timeout, Too Many Requests
        ],
        'git': [
            'timeout', 'timed out', 'connection', 'network', 'unreachable',
            'temporary', 'lock', 'locked', 'busy', 'retry'
        ],
        'push': [
            'timeout', 'timed out', 'connection', 'network', 'unreachable',
            'temporary', 'refused', 'could not resolve', 'dns', 'temporary failure'
        ]
    }
